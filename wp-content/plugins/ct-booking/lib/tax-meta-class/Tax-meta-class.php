...
class Tax_Meta_Class {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme'])){
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = CT_TAX_META_DIR_URL;
  ...
  public function load_scripts_styles() {
    ...
    if (in_array($taxnow,$this->_meta_box['pages'])){
      ...
      //wp_enqueue_style( 'ct-taxonomy-admin-style', CT_TEMPLATE_DIRECTORY_URI . '/css/soap-icon.css' );
  ...
  */
  ...
  public function check_field_date() {
    ...
    if ( $this->has_field( 'date' ) && $this->is_edit_page() ) {
      ...
      wp_enqueue_style( 'tmc-jquery-ui-css', '//ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css' );
      wp_enqueue_script( 'tmc-jquery-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array( 'jquery' ) );
    }
  ...
  public function check_field_time() {
    ...
    if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
      ...
      // Enqueu JQuery UI, use proper version.
      wp_enqueue_style( 'tmc-jquery-ui-css', '//ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css', array(),false,true);
      wp_enqueue_script( 'tmc-jquery-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array( 'jquery' ),false,true );
  ...
  public function show_field_select( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      foreach ( $field['options'] as $key => $value ) {
        echo "<option class='{$key}' value='{$key}'" . selected( in_array( $key, $meta ), true, false ) . ">{$value}</option>";
      }
      echo "</select>";
    $this->show_field_end( $field, $meta );
