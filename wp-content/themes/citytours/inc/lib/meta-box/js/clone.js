...
jQuery( function ( $ )
{
	...
	// Add clones
		.on( 'click', '.add-clone', function ( e )
		{
			...
			if ( $container.find( '.ct_datepicker' ).length ) {
				$container.find( '.ct_datepicker' ).datepicker( 'destroy' );
				$container.find( '.ct_datepicker' ).removeClass( 'hasDatepicker' ).removeAttr( 'id' );
			}
			...
			if ( $container.find( '.ct_datepicker' ).length ) {
				$container.find( '.ct_datepicker' ).datepicker({ dateFormat: 'yy-mm-dd' });
			}
		} )
		// Remove clones
		.on( 'click', '.remove-clone', function ( e )
		{
			...
			$this.closest('.rwmb-clone').trigger( 'remove' ).remove();
			toggleRemoveButtons( $container );
			toggleAddButton( $container )
		} );
