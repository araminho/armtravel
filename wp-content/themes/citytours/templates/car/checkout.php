...
$car_id = $cart->get_field( $uid, 'car_id' );
$date = $cart->get_field( $uid, 'date' );
$time = $cart->get_field( $uid, 'time' );
$pickup_location = $cart->get_field( $uid, 'pickup_location' );
$dropoff_location = $cart->get_field( $uid, 'dropoff_location' );
$cart_car = $cart->get_field( $uid, 'car' );
$adults = $cart_car['adults'];
$kids = $cart_car['kids'];
...
if ( ! ct_get_car_thankyou_page() ) { 
	...
	<form id="booking-form" action="<?php echo esc_url( ct_get_car_thankyou_page() ); ?>">

		<div class="row">

			<div class="col-md-8">

				<?php do_action( 'car_checkout_main_before' ); ?>
			...
			<?php if ( ! empty( $ct_options['pay_paypal'] ) ) : ?>
				<div class="form_title">
					...
					<?php } else { ?>
					<p><?php echo esc_html__( 'You"ll be redirected to paypal to pay for this car', 'citytours' ) ?></p>
					<?php } ?>
				</div>
			...
			<?php endif; ?>
				...
				<div id="policy">
					...
					<?php 
					if ( ! empty( $ct_options['car_terms_page'] ) ) : 
						?>

						<h4><?php echo esc_html__( 'Cancellation policy', 'citytours' ) ?></h4>

						<div class="form-group">
							<label><input name="agree" value="agree" type="checkbox" checked><?php printf( __('By continuing, you agree to the <a href="%s" target="_blank"><span class="skin-color">Terms and Conditions</span></a>.', 'citytours' ), ct_get_permalink_clang( $ct_options['car_terms_page'] ) ) ?></label>
						</div>
				...
				<?php do_action( 'car_checkout_main_after' ); ?>
			...
			<aside class="col-md-4">
				...
				<?php do_action( 'car_checkout_sidebar_before' ); ?>
				...
				<div class="box_style_1">
					...
					<a class="btn_full_outline" href="<?php echo esc_url( get_permalink( $car_id ) ) ?>">
						<i class="icon-right"></i> <?php echo esc_html__( 'Modify your search', 'citytours' ) ?>
					</a>

					<input type="hidden" name="action" value="ct_car_submit_booking">
				...
				<?php do_action( 'car_checkout_sidebar_after' ); ?>
